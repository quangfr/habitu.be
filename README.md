**🌱 Routines — Your week, simplified.**  

A playful, minimalist app to stay consistent with what truly matters — your daily and weekly habits.  
💾 100% offline & private — no signup, no internet, just you.

📅 **Weekly dashboard** — see your score and routines 🟥🟨🟩 at a glance.  
💪 **Easy to use** — with emojis, mood and samples.  
🎯 **Practical tracking** — tap days to mark done, get notifications.  
📲 **Install as an app** — Chrome → ⋮ → “Add to Home screen”.
⚙️ **Fully yours** — customize and get your data anytime.  

✨ *See your progress, one week at a time.*

--

**🌱 Routines — Ta semaine, simplifiée.**  

Une appli ludique et minimaliste pour rester régulier dans ce qui compte vraiment — tes habitudes du quotidien et de la semaine.  
💾 100 % hors ligne et privée — aucun compte, aucun cloud, juste toi.

📅 **Vue hebdo claire** — vois ton score et tes routines 🟥🟨🟩 d’un clin d’œil.  
💪 **Facile à utiliser** — avec émojis, état d'humeur et exemples.  
🎯 **Suivi pratique** — clique sur les jours réalisés, reçois des rappels.  
📲 **Installe-la comme une appli** — Chrome → ⋮ → « Ajouter à l’écran d’accueil ».
⚙️ **Entièrement personnalisable** — personnalise et récupère tes données à tout moment.  

✨ *Vois tes progrès, une semaine après l’autre.*
